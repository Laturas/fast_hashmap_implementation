...
void* _abytepush(arena pushed_arena, u64 bytes) {
    if ((*pushed_arena.size) + bytes > (*pushed_arena.max_size)) {
        debug_print("Arena overflow error!\n");
        printf("Attempted to push object of size %llu onto an arena with only %llu bytes remaining. (The arena's max size was %llu)", 
            bytes,
            (*pushed_arena.max_size) - (*pushed_arena.size),
    ...
    mem_commit(pushed_arena.buffer, bytes + (*pushed_arena.size));

    for (u64 i = 0; i < bytes; i++) {
       _awrite(pushed_arena, i, 0);
    }
    (*pushed_arena.size) += bytes;

    return pushed_arena.buffer + (*pushed_arena.size) - bytes;
}
